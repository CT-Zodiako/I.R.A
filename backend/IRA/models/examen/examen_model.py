...
class Examen(db.Model):
    ...
    programa_id = db.Column(
        db.Integer, db.ForeignKey(Programa.id), nullable=False)
    ...
    resultado_aprendizaje_id = db.Column(
        db.Integer, db.ForeignKey('resultado_aprendizaje.id'))
    estado = db.Column(db.Boolean, default=False)
    notificar = db.Column(db.Boolean, default=False)
    evaluadores_relacion = db.relationship(
        'Evaluador', secondary=examen_evaluador_tabla, back_populates='examenes_evaluador_relacion')

    def __init__(self, programa_id, proyecto_integrador, actividades_formativas, estudiantes, resultado_aprendizaje_id, notificar=False):
    ...
    def to_dict(self):
        return {
            ...
            # Agrega esta línea para obtener los nombres de los evaluadores
            'nombres_evaluadores': self.get_nombres_evaluadores(),
        }
    ...
    def obtener_correos_evaluadores_por_id(self, examen_id):
        ...
        correos_evaluadores = [
            evaluador.correo_evaluador for evaluador in examen.evaluadores_relacion]
